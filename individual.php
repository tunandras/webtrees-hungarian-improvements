	...
	if (WT_LOCALE === "hu" || WT_LOCALE === "vi" || WT_LOCALE === "zh-Hans") { // If language is Hungarian or provides Eastern order
		foreach ($controller->record->getFacts() as $fact) {
			if ($fact->getTag() == 'NAME') {
				...
				$individualsNames = preg_split("/\n/", $individualsNames);
				foreach ($individualsNames as $individualsName) {
					if (strpos($individualsName, "NAME") === FALSE) {
						$nameTag = preg_replace('/^\d\s/', "", $individualsName);
						$nameTag = preg_replace('/_/', "", $nameTag);
						$nameTag = preg_replace('/[^A-Z].*/', "", $nameTag);
						$buffer = preg_replace('/\d\s_?([A-Z])*\s/', "", $individualsName);
		...
		echo '<h3 class="name_one ', $controller->getPersonStyle($controller->record), '"><span>', $full, '</span>'; // First name accordion header
	} else
# END
