...
class FunctionsDb {
	...
	/**
	 * Clear individuals’ fact content in statistics block.
	 *
	 * @param string $content
	 *
	 * @return string
	 */
	public static function clearFieldContent($content) {
		$datarray = explode ("<br>", $content);
		$data = "";
		foreach($datarray as $content) {
			$content = preg_replace ('/\s+/', " ", $content);
			if (strpos($content, '<span dir="auto"></span>')>0)
