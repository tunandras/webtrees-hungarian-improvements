...
case 'INDI': // Individuals, whose name contains the search terms
	...
	foreach ($rows as $row) {
		...
		if ($person->canShowName()) {
			...
			$data[] = array('value' => $row->xref, 'label' => str_replace(array('@N.N.', '@P.N.'), array(I18N::translateContext('Unknown surname', '…'), I18N::translateContext('Unknown given name', '…')), $name) . ', <i>' . $person->getLifeSpan() . '</i>');
		}
	}
	echo json_encode($data);
